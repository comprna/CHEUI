...
Correct signal means to flip every sample list because they are flipped.
"""
            ...
            for read in chunk_pos['read_name'].unique():
                ...
                if len(set(chunk_read['position'])) == 5:
                    ...
                    for kmer in chunk_read['reference_kmer'].unique():
                        ...
                        if len(kmer_sample_signal) > 1:
                            total_event = []
                            for sub_event in kmer_sample_signal:
                                total_event += sub_event.split(',')[::-1]
                            kmer_sample_signal = total_event
                        else:
                            kmer_sample_signal = kmer_sample_signal[0].split(',')[::-1]
...
for chunk in pd.read_csv(eventalign, chunksize=100000, sep='\t'):
    # check how many different contigs there are in the chunk
    contigs = chunk['contig'].unique()
    # go through every different chunk
    for contig in contigs:
