...
def add_noise(signal,
    ...
    for i in enumerate(sequences_np):
        if (i[1] == prev).all() == True:
            ...
            # measure noise per event
            temp_same_motif = np.array(temp_same_motif)
            noises = {}
            modes = {}
            for j in range(0,100,20):
                temp_signals_event = temp_same_motif[:,j:j+20]
                temp_signals_event = temp_signals_event.reshape(len(temp_signals_event)*temp_signals_event.shape[1])
                noises[j] =  stats.median_absolute_deviation(temp_signals_event)
                modes[j] = [min(temp_signals_event),max(temp_signals_event)]
            
            # first clone the same array n times
            
            temp_augmented = np.tile(temp_same_motif,(number_augment,1))
            temp_expected_aug =  np.tile(temp_expected,(number_augment,1))
            
            # create a 0 array same shape as the future augmented data
            noise_np = np.zeros((temp_augmented.shape))
            
            # add noise in chunks based on events
            for noise in noises.keys():
                noise_chunk = np.random.normal(0, noises[noise], (len(temp_augmented), 20))
                noise_np[:,noise:noise+20] = noise_chunk
            
            # Add noise calculated before
            temp_augmented = temp_augmented + noise_np
            
            # use the thresholds to crop to the minimun and maximun values
            for k in range(0,100,20):
                temp_augmented[:,k:k+20][temp_augmented[:,k:k+20] > modes[k][1]] = modes[k][1]
                temp_augmented[:,k:k+20][temp_augmented[:,k:k+20] < modes[k][0]] = modes[k][0]
            
           
            
            augmented_signals += temp_augmented.tolist()
...
if __name__ == '__main__':
    ...
    # gaussian noisse
    augment_mod_signals, augment_mod_distances  = add_noise(mod_rep_2_signal_events,
                                                            multiply_data_times,
    ...
    augment_no_mod_signals, augment_no_mod_distances  = add_noise(no_mod_rep_2_signal_events,
                                                                  multiply_data_times,
    ...
    checkpoint_filepath = '/media/labuser/Data/nanopore/m6A_classifier/results/singleA/singleA30TL_aug_noise_distributions_100/'
