...
#eventalign = '/home/pablo/lib/m6Aclassifier/data/no_mod_rep1_eventalign_test.txt'

out_folder = eventalign[:-4]+'_numpy_sites_AA'
out_counter = eventalign[:-4]+'_numpy_sites_AA/counter.txt'
...
for chunk in pd.read_csv(eventalign, chunksize=500000, sep='\t'):
    ...
    counter = chunk.index[-1] # print the counter in case it run out of memory
    with open(out_counter, 'w') as counter_out:
        print(counter, file=counter_out)
            ...
            for row in chunk_read_A.itertuples():
                if re.findall('A$', row[3]):
                    ...
                    if len(set(chunk_read_POS['position'])) == 5:
                        ...
                        temp_signal = []
                        for kmer in chunk_read_POS['position'].unique():
                            
                            kmer_sample_signal = list(chunk_read[chunk_read['position'] == kmer]['samples'])
                            
                            kmer_sample_signal = [float(i) for i in ','.join(kmer_sample_signal).split(',')]
                            
                            temp_signal.append(kmer_sample_signal)
                            
                        with open(out_folder+'/'+contig+'_'+str(start)+'_'+kmer_name, "ab") as f:
