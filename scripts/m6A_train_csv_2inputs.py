...
from DL_models import build_Jasper, build_deepbinner, build_Jasper_2inputs
...
def parse_chunk(chunk):
    ...
    # parse the sequneces and dwelling time 
    dwell = chunk['dwell']
    sequences = chunk['sequences']
    
    Idx = [0, 20, 40, 60, 80] 
    
    dwell = np.array([np.array(list([xi[i] for i in Idx])) for xi in dwell]) 
    sequences = np.array([np.array(list([xi[i] for i in Idx])) for xi in sequences]) 
    
    combined_s = np.concatenate((dwell,
                                 sequences), 
                                 axis=1)

    # reshape the vectors
    combined_s = combined_s.reshape(len(combined_s), 2, 5)
    
    # transpose the last two dimensions to have each channel with separate information
    combined_s = np.transpose(combined_s, (0, 2, 1))
...
if __name__ == '__main__':
    ...
    # Define model Inout
    input1 = Input(shape=(100, 2))
    input2 = Input(shape=(5, 2))
    
    output = build_Jasper_2inputs(input1, input2, Deep=True)
    ...
    train_path = '/media/labuser/Data/nanopore/m6A_classifier/data/Epinano/doubleAA/test_s.csv'
    ...
    event_distance_test, dwell_sequence_test = parse_chunk(test)
    y_test = test['label']
    ...
    for e in range(1,epochs+1):
        ...
        for chunk in pd.read_csv(train_path, chunksize=100000, sep='\t', converters={'event': eval,
                                                                                     'dwell': eval,
                                                                                     'distances': eval,
                                                                                     'sequences': eval}):
            event_distance_train, dwell_sequence_train = parse_chunk(chunk)
            
            y_train = chunk['label']
            
            #Train the model on batches
            history = model.fit([event_distance_train,
                                 dwell_sequence_train],
                                y_train, 
                                batch_size=125,
                               validation_data=([event_distance_test, dwell_sequence_test],
                                                y_test),
    ...
    lr_probs = model.predict([event_distance_test, dwell_sequence_test ])
