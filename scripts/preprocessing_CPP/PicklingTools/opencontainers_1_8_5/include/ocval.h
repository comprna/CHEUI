...
// First method is preferred, but second method always works
#if defined(OC_DELETE_OPBRACKET) 
   #define OCBRACKETSILLEGALCONST =delete;  // op[] is illegal on const Val!
#else 
   #define OCBRACKETSILLEGALCONST ;   // No implementation for this function
                             // So a link-error instead of a compile-time error
#endif
...
struct Val {
    ...
    // YOU CAN'T CALL a OPERATOR[] on a const Val!  This generates a compiler 
    // error!!!!   It's not a great compiler error, but at least it shows you 
    // are trying to call something you shouldn't!! (Previously, it would 
    // perform a op int_8 conversion, which was worse!).  With newer C++, 
    // it's a deleted function (meaning you can't call... you get a compiler 
    // error).  With older C++, it's a function with no implementation, so you 
    // get a link error
    OC_INLINE Val& operator [] (int_1) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_u1) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_2) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_u2) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_4) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_u4) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_8) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_u8) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_n) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (int_un) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (ALLOW_SIZE_T) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (ALLOW_LONG) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (ALLOW_UNSIGNED_LONG) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (ALLOW_LONG_LONG) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (ALLOW_UNSIGNED_LONG_LONG ) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (bool) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (real_4) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (real_8) const OCBRACKETSILLEGALCONST
    //inline Val& operator [] (complex_8 ) const OCBRACKETSILLEGALCONST
    //inline Val& operator [] (complex_16 ) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (const char*) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (const Str&) const OCBRACKETSILLEGALCONST
    OC_INLINE Val& operator [] (const Val&) const OCBRACKETSILLEGALCONST
    //OC_INLINE Val& operator [] (Tab& t) const;      // doesn't make sense
    //OC_INLINE Val& operator [] (Array<T>& t); // doesn't make sense
