        ...
        for pos in position_single_A[contig]:
            ...
            for read in chunk_pos['read_name'].unique():
                ...
                if len(set(chunk_read['position'])) == 5:
                    ...
                    kmer = chunk_read[chunk_read['position'] == pos-2]['reference_kmer'].unique()[0]
                    if contig+'_'+str(pos)+'_'+kmer in dic_site_: # be careful with the reference and model kmer
                        dic_site_[contig+'_'+str(pos)+'_'+kmer] += [list(chunk_read['samples'])]
                    else:
                        dic_site_[contig+'_'+str(pos)+'_'+kmer] = [list(chunk_read['samples'])]
